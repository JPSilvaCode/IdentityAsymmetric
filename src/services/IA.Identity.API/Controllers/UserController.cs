...
namespace IA.Identity.API.Controllers
{
	...
	public class UserController : MainController
	{
		...
		public async Task<ActionResult> Update([FromRoute] string email, [FromBody] UserUpdate userUpdate)
		{
			if (!ModelState.IsValid) return CustomResponse(ModelState);
		...
		[HttpPost]
		[Route("user/{id:int}/role/{roleId:int}")]
		[AllowAnonymous]
		[ProducesResponseType((int)HttpStatusCode.NoContent)]
		[ProducesResponseType((int)HttpStatusCode.NotFound)]
		public async Task<IActionResult> AssignRoleToUser(int id, int roleId)
		{
			var user = await _authenticationService.UserManager.FindByIdAsync(id.ToString());

			if (user == null) return NotFound();

			var role = await _authenticationService.RoleManager.Roles.FirstOrDefaultAsync(x => x.Id.Equals(roleId));

			if (role == null) return NotFound();

			var currentRolesUser = await _authenticationService.UserManager.GetRolesAsync(user);

			if (currentRolesUser.Any(r => r.Equals(role.Name)))
			{
				AddProcessingError($"Role '{role.Name}' já existe para o usuário");
				return CustomResponse();
			}

			var addResult = await _authenticationService.UserManager.AddToRoleAsync(user, role.Name);
		...
		public async Task<IActionResult> RemoveRoleToUser(int id, int roleId)
		{
			...
			if (!currentRolesUser.Any(r => r.Equals(role.Name)))
			{
				AddProcessingError($"Role '{role.Name}' não existe para o usuário");
				return CustomResponse();
			}

			var addResult = await _authenticationService.UserManager.RemoveFromRoleAsync(user, role.Name);
		...
		public async Task<IActionResult> GetClaimsTouser(int id)
		{
			...
			if (claimsUser == null || claimsUser.Count.Equals(0)) return NotFound();
		...
		[ProducesResponseType((int)HttpStatusCode.NoContent)]
		[ProducesResponseType((int)HttpStatusCode.NotFound)]
		public async Task<IActionResult> AssignClaimToUser([FromRoute] int id, [FromRoute] string type, [FromRoute] string value)
		{
			var user = await _authenticationService.UserManager.FindByIdAsync(id.ToString());
		...
		[ProducesResponseType((int)HttpStatusCode.NoContent)]
		[ProducesResponseType((int)HttpStatusCode.NotFound)]
		public async Task<IActionResult> PutValueClaimToUser([FromRoute] int id, [FromRoute] string type, [FromRoute] string value)
		{
			var user = await _authenticationService.UserManager.FindByIdAsync(id.ToString());
		...
		[ProducesResponseType((int)HttpStatusCode.NoContent)]
		[ProducesResponseType((int)HttpStatusCode.NotFound)]
		public async Task<IActionResult> RemoveClaimFromUser([FromRoute] int id, [FromRoute] string type)
		{
			var user = await _authenticationService.UserManager.FindByIdAsync(id.ToString());
